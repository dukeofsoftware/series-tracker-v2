...
const page: FC<pageProps> = async ({
    ...
    const messages = await db.collection(`chatRooms/${roomId}/messages`).get().then((querySnapshot) => {
        const messages: any = []
    ...
    const user = (await db.collection("users").doc(otherUser[0]).get()).data() as {
        username: string
        displayName?: string
        photoURL?: string
    }
    ...
    return <div className='relative w-full'>
        ...
        <Messages user={
            user
        } serverMessages={messages} roomId={roomId} />
